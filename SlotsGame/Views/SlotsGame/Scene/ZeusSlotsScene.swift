...
class ZeusSlotsScene: SKScene {
    
    var slot: Slot
    
    var baraban1: SlotMachineBaraban!
    var baraban2: SlotMachineBaraban!
    var baraban3: SlotMachineBaraban!
    ...
    override func didMove(to view: SKView) {
        ...
        baraban1 = SlotMachineBaraban(slotSymbols: getSlotSymbols(), symbolBack: "\(slot.symbol)_symbol_bg", size: CGSize(width: 200, height: 730)) {
        }
        ...
        baraban3 = SlotMachineBaraban(slotSymbols: getSlotSymbols(), symbolBack: "\(slot.symbol)_symbol_bg", size: CGSize(width: 200, height: 730)) {
            self.checkWinning()
        }
    ...
    private func checkWinning() {
        ...
        // corners items
        let topLeftCornerItem = atPoint(CGPoint(x: size.width / 2 - 230, y: size.height / 2 + 240)).name ?? ""
        let bottomLeftCornerItem = atPoint(CGPoint(x: size.width / 2 - 230, y: size.height / 2 - 240)).name ?? ""
        let topRightCornerItem = atPoint(CGPoint(x: size.width / 2 + 230, y: size.height / 2 + 240)).name ?? ""
        let bottomRightCornerItem = atPoint(CGPoint(x: size.width / 2 + 230, y: size.height / 2 - 240)).name ?? ""
        
        var hasWin = false
        
        if topLeftCornerItem == centerBarabanCenterItem && bottomRightCornerItem == centerBarabanCenterItem {
        ...
        if topRightCornerItem == centerBarabanCenterItem && bottomLeftCornerItem == centerBarabanCenterItem {
            currentWin += currentBet * 5
            hasWin = true
        }
...
#Preview {
    VStack {
        SpriteView(scene: ZeusSlotsScene(slot: Slot(symbol: "slots_2", isPurchased: true, symbolsCount: 8)))
            .ignoresSafeArea()
    }
}
